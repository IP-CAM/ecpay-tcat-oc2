...
class ControllerShippingTcat extends Controller {
    ...
    public function create_shipping_order() {
        ...
        if ($Result['ResCode'] == 1) { // 成功
            ...
            // update fail status, but don't send notify to customer
            if ($order_info['order_status_id'] != $settings['tcat_order_fail_status_id']) { 
                $comment = '宅配托運運單產生失敗<br />    ';
                if (isset($Result['ResCode'])) {
                    $comment .= '交易狀態代碼: ' . $Result['ResCode']. '<br />    ';
                }
                $this->model_checkout_order->addOrderHistory($order_id, $settings['tcat_order_fail_status_id'], $comment);
            }
        }
